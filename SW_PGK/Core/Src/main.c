...
static bool intruder = false;
static bool flame = false;
static bool motion = false;
static bool arm = false;
uint8_t tx_buffer[20]="Intruder!\n\r";
uint8_t tx2_buffer[20]="Flame!\n\r";
uint8_t tx3_buffer[20]="Motion!\n\r";
uint8_t tx4_buffer[20]="Invalid!\n\r";
uint8_t rx_indx;
uint8_t rx_data[2];
...
void Dis(char* str, uint16_t X, uint16_t Y){
	  SSD1306_Init (); // initialise the display
	  SSD1306_GotoXY (X,Y); // goto 10, 10
	  SSD1306_Puts (str, &Font_11x18, 1); // print Hello
...
int main(void)
{
  ...
  HAL_NVIC_DisableIRQ(EXTI15_10_IRQn);
    ...
    SSD1306_Puts ("ALARM", &Font_11x18, 1); // print Hello
	 	  ...
	 	  snprintf(buffer, sizeof(buffer), "Time: %02d:%02d:%02d, Date: %02d-%02d-%02d\r\n",
	 	  	 ...
	 	  	 if(alarm==true){
	 	  		HAL_GPIO_WritePin(GPIOA, GPIO_PIN_12, GPIO_PIN_SET);
	 	  		 ...
	 	  		 if(flame == true){
	 	  			HAL_UART_Transmit(&huart1, tx2_buffer, 20,HAL_MAX_DELAY);
	 	  			HAL_Delay(100);
	 	  			HAL_UART_Transmit(&huart1, (uint8_t*)buffer, strlen(buffer), HAL_MAX_DELAY);
	 	  			HAL_Delay(5000);
	 	  		 }
	 	  		 if(motion == true){
	 	  			HAL_UART_Transmit(&huart1, tx3_buffer, 20,HAL_MAX_DELAY);
	 	  			HAL_Delay(100);
	 	  			HAL_UART_Transmit(&huart1, (uint8_t*)buffer, strlen(buffer), HAL_MAX_DELAY);
	 	  			HAL_Delay(5000);
	 	  		 }
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart){
	...
	if(huart -> Instance == USART1){
		...
		HAL_UART_Receive_IT(&huart1, rx_data, 1);
		HAL_UART_Transmit(&huart1, rx_data, strlen(rx_data),HAL_MAX_DELAY);
	}
}
/* USER CODE END 4 */
